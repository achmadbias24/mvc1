...
class Mahasiswa extends Controller
{
    ...
    {
        $id = $_POST['pilih'];
        $data['judul'] = 'Daftar Matkul Tersedia';
        $data['cmkl'] = $this->model('Mahasiswa_model')->getMatkul($id);
        $data['id'] = $id;
        $this->view('templates/header', $data);
        $this->view('mahasiswa/matkul', $data);
        $this->view('templates/footer');
    }

    public function tambahMatkul()
    {
        if ($this->model('Mahasiswa_model')->tambahDataMatkul($_POST) > 0) {
            Flasher::setFlash('Data Matkul Mahasiswa', 'berhasil', 'ditambahkan', 'success');
            header('Location: ' . BASEURL . '/mahasiswa');
            exit;
        } else {
            Flasher::setFlash('Data Matkul Mahasiswa', 'gagal', 'ditambahkan', 'danger');
            header('Location: ' . BASEURL . '/mahasiswa');
            exit;
        }
    }
